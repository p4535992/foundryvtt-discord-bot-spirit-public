...
public interface SheetApi {
    ...
    /**
     * ## Returns raw HTML (or a string in a JSON response) for an entity If returning HTML there
     * are options for scale, tab to open (if available), and darkMode. If returning JSON the HTML
     * is untouched.
     *
     ...
     * @param scale    parameter value. Response 200: OK
     */
    @GET
    @Produces({
            "text/plain" })
